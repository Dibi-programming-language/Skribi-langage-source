...
use crate::parse::nodes::id_nodes::{CGet, parse_cget, parse_id_set, IdSet, OpIn, IdGet};
...
fn test_parse_id_simple() {
    ...
    let mut tokens = vec![Token::Identifier(String::from("hello"))].into_iter().collect();
    ...
    let mut tokens = vec![Token::Identifier(String::from("dar"))].into_iter().collect();
    let c_get = parse_cget(&mut tokens).unwrap();
    
    assert_eq!(CGet { name: String::from("dar") }, c_get);
}
